...
const { advanceIndex, crossRankClass, rankValues, slides, oodSlide } = context.globalThis.ResultsCarousel;

assert.equal(slides.length, 3);
assert.equal(advanceIndex(2, 1, 3), 0);
assert.equal(advanceIndex(0, -1, 3), 2);
assert.equal(oodSlide.type, 'ood');
